...
```{r}
additive_model <- lm(Y ~ D + X,data=data)
...
```{r}
interaction_model <- lm(Y ~ D*X,data=data)
...
data |> 
  ...
  filter(D==1) |>
  summarize(
    ATT_est = mean(Y1_interaction_est) - mean(Y0_interaction_est)
  )
```
...
data |> 
  ...
  filter(D==0) |>
  summarize(
    ATU_est = mean(Y1_interaction_est) - mean(Y0_interaction_est)
  )
```
