...
```{r}
lm(score ~ 0 + budget, data=movies)

lm(score ~ 1, data=movies)
```
...
```{r}
lm(score ~ relevel(as.factor(genre),'Comedy'),data=movies)
```
...
```{r}
lm(score ~ year,data=movies)

lm(score ~ as.factor(year),data=movies)
```
...
```{r}
lm(score ~ budget + rating + year,data=movies)
```
...
fit <- lm(score ~ budget + rating + year,data=movies)
