...
```{r}
first_stage <- lm(D ~ Z,iv_data)

'First Stage Results:'
summary(first_stage)

# Predict D using first-stage regression
iv_data <- iv_data |>
  mutate(
    # Predict D from Z
    D_Z = predict(first_stage)
  )

second_stage <- lm(Y ~ D_Z,iv_data)
...
# Take residuals of first stage regression
second_stage <- iv_data |>
  mutate(
    D_U = D - predict(first_stage)
  ) |>
  lm(Y ~ D + D_U,data=_)
...
```{r}
weak_iv_data <- simulate_iv_data(pi=0.01,gamma=0)

iv <- ivreg(Y ~ D | Z, data=weak_iv_data)
...
```{r}
bad_iv_data <- simulate_iv_data(pi=1,gamma=-1)

iv <- ivreg(Y ~ D | Z, data=bad_iv_data)
...
rd_data |> 
  ...
  theme_bw() +
  labs(
    title="Treatment probability jumps at the discontinuity"
  )
```
...
rd_data |> 
  ...
  theme_bw() +
  labs(
    title="Grades drop at the discontinuity"
  )
```

Now... How can we estimate the treatment effect at the discontinuity?

First lets verify that a standard IV estimator does not work (do you see why?)

```{r}
ivreg(Y ~ D | Z, data=rd_data)
```


Instead, we should:

1) Select data in a small window around the discontinuity
2) Use an IV estimator to deal with the fact that the treatment probability changes less than 0 to 1 (a fuzzy RDD design)


However, this still doesn't quite work:

```{r}
bandwidth_data <- filter(rd_data,abs(Z)<0.1)

ivreg(Y ~ D | Z, data= bandwidth_data) |> 
  summary()
```

We also need the instrument to "jump" at the cutoff. 

The simplest version of this is to use an indicator for Z>0 as our instrument:

```{r}
ivreg(Y ~ D | (Z>0), data=rd_data |> filter(abs(Z)<0.1)) |> 
  summary()
```

A more sophisticated approach is to predict treatment using a local linear relationship on each side of the cutoff:

```{r}
ivreg(Y ~ D | (Z>0)*Z, data=rd_data |> filter(abs(Z)<0.1)) |> 
  summary()
```
We can also use weights to prioritize observations closer to the cutoff:
...
rd_data |> 
  ...
  ivreg(Y ~ D | (Z>0)*Z,data=_, weights=w) |> 
  summary()
```
